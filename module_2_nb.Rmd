...
We can even do this with modeling and other operations. This extracts the
coefficients from a model run for each group.
...
model_variables %>% 
  plot_ly(x = ~race, y = ~ age) %>% 
  add_boxplot(color =~ race)
```
...
```{r plotly2}
init = glm(award_total_amount >= 1000000 ~ age*race, 
           data = model_variables, 
           family = binomial)
model_variables %>% 
  modelr::add_predictions(init, type = 'response') %>% 
  plot_ly(x = ~age, y = ~ pred) %>% 
  add_lines(color =~ race, line = list(shape = "spline")) %>% 
